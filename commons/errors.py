...
def add_dash_to_formatted_errors_message(formatted_errors):
    """
    Receives formatted errors and replaces spaces and underline with dash in the error message
    """
    i = 0
    for error in formatted_errors["errors"]:
        if "token_class" in error or "token_type" in error:
            formatted_errors["errors"][i]["token_class"]["message"] = ErrorDetail(
                string=error["token_class"]["message"]
                .replace(" ", "-")
                .replace("_", "-"),
                code=error["token_class"]["code"],
            )
            formatted_errors["errors"][i]["token_type"]["message"] = ErrorDetail(
                string=error["token_type"]["message"]
                .replace(" ", "-")
                .replace("_", "-"),
                code=error["token_type"]["code"],
            )
            formatted_errors["errors"][i]["message"]["message"] = ErrorDetail(
                string=error["message"]["message"].replace(" ", "-").replace("_", "-"),
                code=error["message"]["code"],
            )
        else:
            formatted_errors["errors"][i]["message"] = ErrorDetail(
                string=error["message"].replace(" ", "-").replace("_", "-"),
                code=error["code"],
            )
        i += 1
...
class ErrorFormatter:
    ...
    def __call__(self):
        ...
        formatted_errors = add_dash_to_formatted_errors_message(formatted_errors)
